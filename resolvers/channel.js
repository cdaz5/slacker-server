...
export default {
  Mutation: {
    getOrCreateChannel: requiresAuth.createResolver(async (parent, { teamId, members }, { user, models }) => {
      ...
      const [data, result] = await models.sequelize.query(
        ...
        having array_agg(pc.user_id) @> Array[${allMemebers.join(',')}] and count(pc.user_id) = ${
  allMemebers.length
};
      `,
        { raw: true },
